...
* Windows (MSVC) - add lib/Remotery.c and lib/Remotery.h to your program. Set
include directories to add Remotery/lib path. The required library ws2_32.lib
should be picked up through the use of the #pragma comment(lib, "ws2_32.lib")
directive in Remotery.c.

* Mac OS X (XCode) - simply add lib/Remotery.c and lib/Remotery.h to your
program.

* Linux (GCC) - add the source in lib folder. Compilation of the code requires
-pthreads for library linkage. For example to compile the same run: cc
lib/Remotery.c sample/sample.c -I lib -pthread -lm

You can define some extra macros to modify what features are compiled into
Remotery. These are documented just below this comment.
...
// Help performance of the server sending data to the client by marking this
// machine as little-endian
...
// Time in seconds between each resync to compensate for drifting between GPU &
// CPU timers, effects of power saving, etc. Resyncs can cause stutter, lag
// spikes, stalls. Set to 0 for never. Affects OpenGL & D3D11
...
// Allows macros to be written that can work around the inability to do:
// #define(x) #ifdef x with the C preprocessor.
#if RMT_ENABLED
...
// Public interface is written in terms of these macros to easily enable/disable
// itself
...
typedef enum rmtError {
    RMT_ERROR_NONE,
    RMT_ERROR_RECURSIVE_SAMPLE,  // Not an error but an internal message to
                                 // calling code

    // System errors
    RMT_ERROR_MALLOC_FAIL,     // Malloc call within remotery failed
    RMT_ERROR_TLS_ALLOC_FAIL,  // Attempt to allocate thread local storage
                               // failed
    RMT_ERROR_VIRTUAL_MEMORY_BUFFER_FAIL,  // Failed to create a virtual memory
                                           // mirror buffer
    RMT_ERROR_CREATE_THREAD_FAIL,  // Failed to create a thread for the server

    // Network TCP/IP socket errors
    RMT_ERROR_SOCKET_INIT_NETWORK_FAIL,  // Network initialisation failure (e.g.
                                         // on Win32, WSAStartup fails)
    RMT_ERROR_SOCKET_CREATE_FAIL,  // Can't create a socket for connection to
                                   // the remote viewer
    RMT_ERROR_SOCKET_BIND_FAIL,    // Can't bind a socket for the server
    RMT_ERROR_SOCKET_LISTEN_FAIL,  // Created server socket failed to enter a
                                   // listen state
    RMT_ERROR_SOCKET_SET_NON_BLOCKING_FAIL,  // Created server socket failed to
                                             // switch to a non-blocking state
    ...
    RMT_ERROR_SOCKET_ACCEPT_FAIL,   // Server failed to accept connection from
                                    // client
    RMT_ERROR_SOCKET_SEND_TIMEOUT,  // Timed out trying to send data
    RMT_ERROR_SOCKET_SEND_FAIL,     // Unrecoverable error occured while
                                    // client/server tried to send data
    RMT_ERROR_SOCKET_RECV_NO_DATA,  // No data available when attempting a
                                    // receive
    RMT_ERROR_SOCKET_RECV_TIMEOUT,  // Timed out trying to receive data
    RMT_ERROR_SOCKET_RECV_FAILED,   // Unrecoverable error occured while
                                    // client/server tried to receive data

    // WebSocket errors
    RMT_ERROR_WEBSOCKET_HANDSHAKE_NOT_GET,     // WebSocket server handshake
                                               // failed, not HTTP GET
    RMT_ERROR_WEBSOCKET_HANDSHAKE_NO_VERSION,  // WebSocket server handshake
                                               // failed, can't locate WebSocket
                                               // version
    RMT_ERROR_WEBSOCKET_HANDSHAKE_BAD_VERSION,  // WebSocket server handshake
                                                // failed, unsupported WebSocket
                                                // version
    RMT_ERROR_WEBSOCKET_HANDSHAKE_NO_HOST,      // WebSocket server handshake
                                                // failed, can't locate host
    RMT_ERROR_WEBSOCKET_HANDSHAKE_BAD_HOST,     // WebSocket server handshake
                                             // failed, host is not allowed to
                                             // connect
    RMT_ERROR_WEBSOCKET_HANDSHAKE_NO_KEY,  // WebSocket server handshake failed,
                                           // can't locate WebSocket key
    RMT_ERROR_WEBSOCKET_HANDSHAKE_BAD_KEY,      // WebSocket server handshake
                                                // failed, WebSocket key is
                                                // ill-formed
    RMT_ERROR_WEBSOCKET_HANDSHAKE_STRING_FAIL,  // WebSocket server handshake
                                                // failed, internal error, bad
                                                // string code
    RMT_ERROR_WEBSOCKET_DISCONNECTED,  // WebSocket server received a disconnect
                                       // request and closed the socket
    RMT_ERROR_WEBSOCKET_BAD_FRAME_HEADER,  // Couldn't parse WebSocket frame
                                           // header
    RMT_ERROR_WEBSOCKET_BAD_FRAME_HEADER_SIZE,  // Partially received wide frame
                                                // header size
    RMT_ERROR_WEBSOCKET_BAD_FRAME_HEADER_MASK,  // Partially received frame
                                                // header data mask
    ...
    RMT_ERROR_SEND_ON_INCOMPLETE_PROFILE,  // An attempt was made to send an
                                           // incomplete profile tree to the
                                           // client

    // CUDA error messages
    RMT_ERROR_CUDA_DEINITIALIZED,  // This indicates that the CUDA driver is in
                                   // the process of shutting down
    RMT_ERROR_CUDA_NOT_INITIALIZED,  // This indicates that the CUDA driver has
                                     // not been initialized with cuInit() or
                                     // that initialization has failed
    RMT_ERROR_CUDA_INVALID_CONTEXT,  // This most frequently indicates that
                                     // there is no context bound to the current
                                     // thread
    RMT_ERROR_CUDA_INVALID_VALUE,    // This indicates that one or more of the
                                     // parameters passed to the API call is not
                                     // within an acceptable range of values
    RMT_ERROR_CUDA_INVALID_HANDLE,   // This indicates that a resource handle
                                     // passed to the API call was not valid
    RMT_ERROR_CUDA_OUT_OF_MEMORY,  // The API call failed because it was unable
                                   // to allocate enough memory to perform the
                                   // requested operation
    RMT_ERROR_ERROR_NOT_READY,  // This indicates that a resource handle passed
                                // to the API call was not valid

    // Direct3D 11 error messages
    RMT_ERROR_D3D11_FAILED_TO_CREATE_QUERY,  // Failed to create query for
                                             // sample

    // OpenGL error messages
    RMT_ERROR_OPENGL_ERROR,  // Generic OpenGL error, no need to expose detail
                             // since app will need an OpenGL error callback
                             // registered
...
typedef enum rmtSampleFlags {
    ...
    // Search parent for same-named samples and merge timing instead of adding a
    // new sample
    RMTSF_Aggregate = 1,
...
#define rmt_Settings() RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_Settings(), NULL)

#define rmt_CreateGlobalInstance(rmt)                             \
    RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_CreateGlobalInstance(rmt), \
                     RMT_ERROR_NONE)
...
#define rmt_LogText(text) RMT_OPTIONAL(RMT_ENABLED, _rmt_LogText(text))
...
#define rmt_EndCPUSample() RMT_OPTIONAL(RMT_ENABLED, _rmt_EndCPUSample())
...
typedef struct rmtCUDABind {
    ...
    // NOTE: These are named differently to the CUDA functions because the CUDA
    // API has a habit of using macros to point function calls to different
    // versions, e.g. cuEventDestroy is a macro for cuEventDestroy_v2.
    void *CtxSetCurrent;
    void *CtxGetCurrent;
    void *EventCreate;
    void *EventDestroy;
...
#define rmt_BindCUDA(bind) RMT_OPTIONAL(RMT_USE_CUDA, _rmt_BindCUDA(bind))
...
#define rmt_EndD3D11Sample() RMT_OPTIONAL(RMT_USE_D3D11, _rmt_EndD3D11Sample())

#define rmt_BindOpenGL() RMT_OPTIONAL(RMT_USE_OPENGL, _rmt_BindOpenGL())

#define rmt_UnbindOpenGL() RMT_OPTIONAL(RMT_USE_OPENGL, _rmt_UnbindOpenGL())
...
#define rmt_UnbindMetal() RMT_OPTIONAL(RMT_USE_METAL, _rmt_UnbindMetal());
...
#define rmt_EndMetalSample() RMT_OPTIONAL(RMT_USE_METAL, _rmt_EndMetalSample())
...
struct rmt_EndCPUSampleOnScopeExit {
    ~rmt_EndCPUSampleOnScopeExit() { _rmt_EndCPUSample(); }
};
...
struct rmt_EndCUDASampleOnScopeExit {
    ...
    ~rmt_EndCUDASampleOnScopeExit() { _rmt_EndCUDASample(stream); }
    void *stream;
};
#endif
#if RMT_USE_D3D11
...
struct rmt_EndD3D11SampleOnScopeExit {
    ~rmt_EndD3D11SampleOnScopeExit() { _rmt_EndD3D11Sample(); }
};
#endif
...
struct rmt_EndOpenGLSampleOnScopeExit {
    ~rmt_EndOpenGLSampleOnScopeExit() { _rmt_EndOpenGLSample(); }
};
#endif
...
RMT_API void _rmt_BeginCPUSample(rmtPStr name, rmtU32 flags,
                                 rmtU32 *hash_cache);
RMT_API void _rmt_EndCPUSample(void);

#if RMT_USE_CUDA
RMT_API void _rmt_BindCUDA(const rmtCUDABind *bind);
RMT_API void _rmt_BeginCUDASample(rmtPStr name, rmtU32 *hash_cache,
                                  void *stream);
