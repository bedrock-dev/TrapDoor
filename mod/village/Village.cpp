...
void Village::showVillagerStatus() {
    auto *map = reinterpret_cast<std::unordered_map<
        trapdoor::ActorUniqueID, std::vector<std::weak_ptr<mod::POIInstance>>,
        trapdoor::ActorUniqueIDHash> *>((char *)this + 96);
    const char *icons[3] = {"☾", "⍾", "☕"};
...
bool Village::printVillagerInfo(trapdoor::Actor *player, trapdoor::Actor *v) {
    ...
    for (auto &villager : *map) {
        ...
        if (actor && v && actor == v) {
            ...
            std::string particle = "minecraft:campfire_smoke_particle";
            if (bed) {
                trapdoor::spawnParticle(bed->poiPos.toVec3() +
                                            Vec3(0.5f, 1.1f, 0.5f),
                                        particle, player->getDimensionID());
                builder.text("Bed position: ").pos(bed->poiPos);
            }

            if (alarm) {
                trapdoor::spawnParticle(alarm->poiPos.toVec3() +
                                            Vec3(0.5f, 1.1f, 0.5f),
                                        particle, player->getDimensionID());
                builder.text("\nAlarm position: ").pos(alarm->poiPos);
            }
            if (work) {
                trapdoor::spawnParticle(work->poiPos.toVec3() +
                                            Vec3(0.5f, 1.1f, 0.5f),
                                        particle, player->getDimensionID());
...
void VillageHelper::insert(Village *village) { villageList.insert(village); }
...
void VillageHelper::tick() {
    rmt_ScopedCPUSample(VILLAGE_TICK, 0) if (gameTick % 40 == 0) {
    ...
    if (!component) {
        ...
        if (actor->getActorId() == "villager_v2") {
            for (auto villages : this->villageList) {
                if (villages->printVillagerInfo(player, actor))
                    return;
            }
            trapdoor::warning(player, LANG("village.error.notDweller"));
        } else {
