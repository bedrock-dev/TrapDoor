...
void checkPlayerInFrozen() {
  frozenCheckTick++;
  if (frozenCheckTick % 100 == 0) {
    printf("check\n");
    if (trapdoor::bdsMod->getLevel()->noPlayerExists()) {
      tickStatus = WorldTickStatus::Normal;
      L_INFO("no player exists in server, change level status from frozen to "
             "normal");
    }
  }
}
...
THook(void, MSSYM_B1QA4tickB1AE11ServerLevelB2AAA7UEAAXXZ,
  ...
  case mod::tick::Normal:
    ...
    if (mod::tick::gameProfiler.inProfiling || mod::tick::isMSPTing) {
      ...
      if (mod::tick::isMSPTing) {
        //发送mspt信息
        isRedstoneTick =
            modInstance->getLevel()->getDimFromID(0)->isRedstoneTick();
  ...
  case mod::tick::Forward:
    ...
    isRedstoneTick = modInstance->getLevel()->getDimFromID(0)->isRedstoneTick();
    trapdoor::broadcastMsg(trapdoor::LANG("tick.fw.end"),
                           mod::tick::forwardTickNum, isRedstoneTick);
...
THook(void, MSSYM_B1QA9tickWorldB1AE12ServerPlayerB2AAE13UEAAHAEBUTickB3AAAA1Z,
      trapdoor::Actor *p, void *tick) {
  original(p, tick);
}

// Dimension::tick
THook(void, MSSYM_B1QA4tickB1AA9DimensionB2AAA7UEAAXXZ, void *dim) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
    original(dim);
    TIMER_END
...
THook(
    ...
    void *levelChunk, trapdoor::BlockSource *blockSource, size_t *tick) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
...
THook(void,
      ...
      void *levelChunk, void *blockSource, INT64 a3, int a4) {
...
THook(
    ...
    void *levelChunk, void *blockSource) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
...
THook(
    ...
    void *queue, trapdoor::BlockSource *source, const int *until, int max,
    bool instalTick) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
...
// Dimension::tick
THook(void, MSSYM_B1QE12tickRedstoneB1AA9DimensionB2AAA7UEAAXXZ, void *dim) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
    original(dim);
    TIMER_END
...
THook(void,
      ...
      void *entitySystem, void *registry) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
...
// pendingAdd
THook(void, MSSYM_B1QE18processPendingAddsB1AE17CircuitSceneGraphB2AAA7AEAAXXZ,
      void *graph) {
  if (mod::tick::gameProfiler.inProfiling) {
    TIMER_START
    original(graph);
...
//实体性能分析的钩子函数
THook(void, MSSYM_B1QA4tickB1AA5ActorB2AAA4QEAAB1UE16NAEAVBlockSourceB3AAAA1Z,
      trapdoor::Actor *actor, trapdoor::BlockSource *blockSource) {
  if (mod::tick::getActorProfiler().inProfiling) {
