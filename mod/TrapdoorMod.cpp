...
namespace mod {
	...
	void TrapdoorMod::registerCommands() {
		...
		commandManager.registerCmd("os", "显示服务器信息")->EXE({
			TrapdoorMod::printOSInfo(player);
		});
		commandManager.registerCmd("cl", "计算器", Any, ArgType::STR)->EXE({
			...
			variables["y"] = pos.y - 1;
			variables["z"] = pos.z;
			auto cx = pos.x < 0 ? pos.x - 15 : pos.x;
			auto cz = pos.z < 0 ? pos.z - 15 : pos.z;
			variables["cx"] = cx / 16;
			variables["cz"] = cz / 16;
			functions f;
			std::string s = holder->getString();
			std::string origin = s;
			s = toLowerString(s);
			stringReplace(s, "--", "+");
			stringReplace(s, "and", "&&");
			...
			oss.precision(22);
			double x1 = cpp_eval::eval<double>(s.c_str(), variables, f);
			...
			if(x1==-0)
				x1 = 0;
			oss << x1;
			evalMsg("%s=§l§b%s", origin.c_str(), oss.str().c_str());
		});
	}
