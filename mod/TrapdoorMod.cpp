...
namespace mod {
	...
	void TrapdoorMod::registerCommands() {
		...
		commandManager.registerCmd("mspt", "show mspt & tps")->EXE({
			tick::mspt();
		});
		//功能开关命令
		commandManager.registerCmd("func", "开启/关闭部分功能")
			->then(
				ARG("hc", "开启/关闭漏斗计数器", BOOL,
					{
			...
			->then(ARG("cs", "开启/关闭区块显示", BOOL, {
				this->playerFunctions.setAble(player->getNameTag(),
											  holder->getBool());
		...
		//便捷模式切换
		commandManager.registerCmd("o", "switch to spectator mode")->EXE({
			player->setGameMode(3);
			broadcastMsg("设置玩家[%s]为观察者模式",
						 player->getNameTag().c_str());
		});

		commandManager.registerCmd("s", "switch to survival mode")->EXE({
			player->setGameMode(0);
			broadcastMsg("设置玩家[%s]为生存模式",
						 player->getNameTag().c_str());
		});

		commandManager.registerCmd("c", "switch to creative mode")->EXE({
		...
		commandManager.registerCmd("dbg", "显示一些调试信息")->EXE({
			player->printInfo();
		});
