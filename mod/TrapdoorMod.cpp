...
void TrapdoorMod::registerCommands() {
  ...
  commandManager.registerCmd("self", "command.self.desc")
      ->then(ARG("chunk", "command.self.chunk.desc", BOOL,
                 {
      ...
      ->then(
          ARG("rs", "command.self.rs.desc", BOOL,
              {
                ...
                info(player, LANG("command.self.rs.set"), holder->getBool());
              }))
...
void TrapdoorMod::useOnHook(Actor *player, const std::string &itemName,
                            BlockPos &pos, unsigned int facing, const Vec3 &v) {
  ...
  if (itemName == "Bone" && this->spawnHelper.isEnable()) {
    ...
    //        block->getName().c_str(), block->getVariant());
    fflush(stdout);
  }
}

CommandPermissionLevel
TrapdoorMod::resetVanillaCommandLevel(const std::string &name,
