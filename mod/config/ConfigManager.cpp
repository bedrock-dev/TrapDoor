...
bool ConfigManager::initialize(const std::string &configFileName) {
    ...
    if (!this->readConfigFile(configFileName))
        return false;
    if (!this->readEULA())
        return false;
    if (!this->readCommandConfig())
        return false;
    if (!this->readLowLevelVanillaCommands())
        return false;
    if (!this->readServerConfig())
        return false;
    if (!this->readVillageConfig())
        return false;
    if (!this->readFunctionConfig())
        return false;
    if (!this->readSelfConfig())
        return false;
    return true;
}
...
bool ConfigManager::readServerConfig() {
    ...
    try {
        ...
        this->serverConfig.crashServerToken =
            config["crashServerToken"].get<std::string>();
...
bool ConfigManager::readVillageConfig() {
    ...
    try {
        ...
        this->villageHelperConfig = {str2color(boundColor),
                                     str2color(spawnColor), str2color(poiQuery),
                                     centerParticle};
